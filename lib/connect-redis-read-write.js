...
module.exports = function (session) {
  ...
  function RedisStore (options) {
    ...
    if(options.split){      
      ...
      options.host = options.w_host;
      options.port = options.w_port;
      this.w_client = redis.createClient(options);
    }else{
    ...
    if(options.logErrors){
      ...
      if(this.w_client){
        this.w_client.on('error', options.logErrors);
      }
    }
    ...
    if (options.pass) {
      ...
      if(this.w_client){
        this.w_client.auth(options.pass, function (err) {
          if (err) {
            throw err;
          }
        });
    ...
    if ('db' in options) {
      ...
      if(this.w_client){
        self.w_client.select(options.db);
        self.w_client.on('connect', function () {
          self.w_client.select(options.db);
        });
      }
    }
    if(this.w_client){
      self.w_client.on('error', function (er) {
        debug('Redis w_client returned err', er);
        self.emit('disconnect', er);
      });
  
      self.w_client.on('connect', function () {
        self.emit('connect w_client');
      });
    }
  ...
  RedisStore.prototype.set = function (sid, sess, fn) {
    ...
    store.w_client.set(args, function (er) {
      if (er) return fn(er);
      debug('SET complete');
  ...
  RedisStore.prototype.touch = function (sid, sess, fn) {
    ...
    store.w_client.expire(psid, ttl, function (er) {
      if (er) return fn(er);
